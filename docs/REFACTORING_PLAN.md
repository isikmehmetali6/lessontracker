...
- [x] **4a.2** Cache politikası + ölçüm notu — *`MoodleImageCacheManager` (`core/utils/moodle_image_cache_manager.dart`) eklendi: Moodle avatar/ders kapağı görselleri artık `DefaultCacheManager`'ın 30 gün/200 nesne varsayılanı yerine ayrı, kısıtlı bir politika kullanıyor (14 gün taze kalma, maks. 100 nesne) — bu görseller yeniden indirilebilir ve az kritik olduğu için disk kullanımını sınırlamak amaçlı. `moodle_courses_tab.dart`'taki iki `CachedNetworkImage`/`CachedNetworkImageProvider` çağrısı bu manager'a bağlandı. `storage_screen.dart`'ın "derin temizlik" (`_deepClean`) adımı artık hem bu cache'i hem `DefaultCacheManager`'ı temizliyor — önceden yalnızca `DefaultCacheManager` temizleniyordu, oysa uygulamanın görselleri artık onu hiç kullanmıyordu (temizlik görünürde çalışıp aslında hiçbir şey silmiyordu). analyze 0, regresyon yok — 11.08.2026*
- [x] **4a.3** Repo lifecycle standardı (opsiyonel) — *Taranan sonuç: 10 repository'den 7'si (`Absence/Course/Deadline/File/Grade/Note/StudySession`) `factory` + `_internal()` singleton deseni kullanıyor, 3'ü (`MoodleAccount/MoodleCache/PlannerEvent`) düz kurucu. Bu rastgele değilmiş: singleton'lar `_dbHelper.isWeb` dalında kullanılan bir `_xInMemory` web-fallback cache'i taşıyor — her `Repo()` çağrısında sıfırlanmaması için paylaşılan örnek şart. Düz olan 3'ünde ise ne in-memory state ne stream var; zaten stateless, singleton'a gerek yok (ve Faz 2.0 DI hedefiyle de daha uyumlu). **Standart:** state'i (web fallback cache) koruması gereken repo singleton olur, geri kalanı düz kurucu kalır — kod değişmedi, her sınıfa bu gerekçeyi açıklayan tek satır doc-comment eklendi. analyze 0 — 11.08.2026*
